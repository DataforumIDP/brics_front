...
export async function userReg() {
  ...
  if (!data.check) {
    ...
    $(".modal__btn").before(
      `<h3 class="modal__error">${
        getLang() == "ru"
          ? "Согласие на обработку данных является обязательным!"
          : "Consent to the processing of personal data is mandatory!"
      }</h3>`
    );
    openModal(".modal");
    return;
  ...
  if (err) {
    ...
    Object.values(err.data.errors).map((item) => {
      $(".modal__btn").before(
        `<h3 class="modal__error">${item[getLang()]}</h3>`
      );
    });
  ...
  $(".modal__error").remove();
  $(".modal__title").text(
    getData("type") == "attendees"
      ? titles.success_a[getLang()]
      : titles.success_t[getLang()]
  );
  openModal(".modal");

  await awaiting(2);
  ym(97687287, "reachGoal", "participant_registration");
  location.reload();
}
  ...
  const result = await new Promise(async (resolve) => {
    try {
      const result = await axios.post(
        `https://brics.wpdataforum.ru/api/attendees`,
        data
      );
